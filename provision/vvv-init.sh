...
echo " * Creating public_html, web, wp folder if it doesn't exist already"
noroot mkdir -p "${VVV_PATH_TO_SITE}/public_html"
noroot mkdir -p "${VVV_PATH_TO_SITE}/public_html/web"
noroot mkdir -p "${VVV_PATH_TO_SITE}/public_html/web/wp"

cd "${VVV_PATH_TO_SITE}/public_html/web/wp"
...
PHP
  ...
  if ! $(noroot wp core is-installed ); then
    ...
    if [ -f "${VVV_PATH_TO_SITE}/public_html/web/wp-content/database.sql" ]; then
      echo " * Found a database backup at public_html/web/wp-content/database.sql. Restoring the site"
      ...
      noroot wp db import "${VVV_PATH_TO_SITE}/public_html/web/wp-content/database.sql"
      ...
      echo " * Installing using wp core install --url=\"${DOMAIN}\" --title=\"${SITE_TITLE}\" --admin_name=\"${ADMIN_USER}\" --admin_email=\"${ADMIN_EMAIL}\" --admin_password=\"${ADMIN_PASSWORD}\" --path=\"${VVV_PATH_TO_SITE}/public_html/web\""
      ...
      if [ "${WP_TYPE}" = "subdomain" ]; then
        echo " * Running Multisite install using wp core multisite-install --subdomains --url=\"${DOMAIN}\" --title=\"${SITE_TITLE}\" --admin_name=\"${ADMIN_USER}\" --admin_email=\"${ADMIN_EMAIL}\" --admin_password=\"${ADMIN_PASSWORD}\" --path=\"${VVV_PATH_TO_SITE}/public_html/web\""
        ...
        echo " * Running Multisite install using wp core ${INSTALL_COMMAND} --url=\"${DOMAIN}\" --title=\"${SITE_TITLE}\" --admin_name=\"${ADMIN_USER}\" --admin_email=\"${ADMIN_EMAIL}\" --admin_password=\"${ADMIN_PASSWORD}\" --path=\"${VVV_PATH_TO_SITE}/public_html/web\""
